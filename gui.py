...
class CrawlWindows(QWidget):
	...
	def __init__(self):
		...
		# self.setLayout(self.v_layout)py 
	...
	def setupUi(self):
		...
		self.listViewAreaList = QtWidgets.QListView(self)
		font = QtGui.QFont()
		...
		self.btnDelete.clicked.connect(self.DeleteSheet)
	...
	def retranslateUi(self):
		...
		self.label_5.setText(_translate("Dialog", "FILE LIST"))
		...
		self.list_widget.addItem('New Sheet')
		header_labels = ["ID", "Title", "Date Updated", "Street", "Suburb", "State", "Postcode", "Price",
						 "Price Range", "Agency Company", "Agency Contact", "Property", "Land Area", "Floor Area", "Car Spaces", "Zoning", "Status", "Highlights", "Description", "Link"]
	...
	def DeleteSheet(self) :
		...
		gc.del_spreadsheet(self.files[selected - 1]['id'])
		self.files.pop(selected - 1)
		self.list_widget.takeItem(selected)

		#print(str(self.list_widget.currentItem().text()))
	...
	def AddSheet(self) :
		...
		spreadsheet = gc.create(self.lineEditName.text())
		...
		while True:
			if not self.gui.Q.empty():
				...
				if record['type'] == 1:
					...
					price = isset('price', data['details'])
					if 'Please contact agent' in price or 'Contact Agent' in price:
						data['details']['price'] = price = ''
					data['rowNum'] = row
					self.gui.database[id] = data
					status = self.gui.channelParams[self.gui.channel]
					...
					#header_labels = ["ID", "Title", "Date Updated", "Street", "Suburb", "State", "Postcode", "Price", "Price Range", "Agency Company", "Agency Contact", "Property", "Land Area", "Floor Area", "Carspaces", "Zoning", "Status"]
					...
					self.gui.tableViewResult.model().item(
						rowNum, 18).setText(data['description'].replace("<br/>", "\n"))
...
class ListThread(QThread):
	...
	def run(self):
			...
			self.gui.list_widget.addItem('New Sheet')
			self.gui.files = []
			for file in gc.list_spreadsheet_files():
				self.gui.files.append(file)
				self.gui.list_widget.addItem(file['name'])
...
class SaveThread(QThread):
	...
	def run(self):
		self.gui.btnSave.setEnabled(False)
		now = datetime.now().strftime("%y%m%d%H%M%S%f")
		if self.gui.list_widget.currentRow() <= 0 :
			spreadsheet = gc.create(f'realcommercial-{now}')
			self.gui.list_widget.addItem(f'realcommercial-{now}')
			self.gui.files.append({'id': spreadsheet.id})
		else :
			spreadsheet = gc.open(self.gui.list_widget.currentItem().text().strip())
		print("OK")
		print(spreadsheet)
  
		file_id = spreadsheet.id
		gc.insert_permission(file_id, 'softprom0109@gmail.com', 'user', 'writer', False)
		gc.insert_permission(file_id, 'franktripodisa@gmail.com', 'user', 'writer', False)
		...
		header = ["ID", "Title", "Date Updated", "Street", "Suburb", "State", "Postcode", "Price",
								"Price Range", "Agency Company", "Agency Contact", "Property", "Land Area", "Floor Area", "Car Spaces", "Zoning", "Status", "Highlights", "Description", "Link"]
		...
		for row, value in enumerate(values):
			olds[value[0]] = value
			olds[value[0]].append(row + 2)

		status = self.gui.channelParams[self.gui.channel]

		for value in self.gui.database.values():
			flag = 0
			data = value
			detail = isset('detail', data, [])

			updatedAt = isset('lastUpdatedAt', detail)
			address = isset('address', data)
			street = isset("streetAddress",address)
			suburb = isset("suburb", address)
			state = isset("state",address)
			postcode = isset("postcode", address)
			...
			if updatedAt != "" and updatedAt <= self.gui.datefrom or updatedAt >= self.gui.dateto + "T23:59:59Z":
				continue
			...
			if (id in olds):
				if (olds[id][2] < updatedAt):
					worksheet.delete_row(olds[id][col_cnt])
					worksheet.insert_row(item, olds[id][col_cnt])
			else:
				data_to_upload.append(item)

		# print (data_to_upload)
		# Upload data to the Google Sheet
		worksheet.insert_rows(data_to_upload, len(values) + 2)  # Change the row number as needed
		self.gui.btnSave.setEnabled(True)
